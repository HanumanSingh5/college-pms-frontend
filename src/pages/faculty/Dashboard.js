...
export default function FacultyDashboard() {
  ...
  const load = () => {
    axios.get('http://localhost:5000/api/faculty/stats',    h).then(r => setStats(r.data));
    axios.get('http://localhost:5000/api/faculty/projects', h).then(r => setProjects(r.data));
  };
  ...
  const openSelectModal = (project) => {
    ...
    setEditedDef(
      ...
      project.definitions?.[idx]?.description || ''
    );
  };
  ...
  const finalizeDefinition = async (e) => {
    ...
    try {
      await axios.put(
        'http://localhost:5000/api/faculty/project/' + selectModal._id + '/select-definition',
      ...
      <div className="layout">
        ...
        <div className="main-content">
          ...
          <div style={{ borderBottom:'1px solid #e5e7eb', marginBottom:20, display:'flex', gap:4 }}>
            <button style={tabStyle('projects')}    onClick={() => setTab('projects')} type="button">
              📁 My Projects
            </button>
            <button style={tabStyle('students')}    onClick={() => setTab('students')} type="button">
              👥 Student Details
            </button>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                    <div style={{ flex:1 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8, flexWrap:'wrap' }}>
                        ...
                        <span style={{ background:'#eff6ff', color:'#1d4ed8', padding:'2px 10px', borderRadius:20, fontSize:12 }}>
                          Group: {p.groupNo || '-'}
                        </span>
                        <span className={
                          p.definitionStatus === 'finalized' ? 'badge badge-success' :
                          p.definitionStatus === 'submitted' ? 'badge badge-info' : 'badge badge-warning'
                        }>
                          {p.definitionStatus === 'finalized' ? '✅ Finalized' :
                           p.definitionStatus === 'submitted' ? '📤 ' + (p.definitions?.length || 0) + ' Definitions Submitted' : '⏳ Pending'}
                        </span>
                      </div>

                      <div style={{ display:'flex', gap:8, marginBottom:10, flexWrap:'wrap' }}>
                        {p.frontend && (
                          <span style={{ background:'#eff6ff', color:'#1d4ed8', padding:'2px 10px', borderRadius:6, fontSize:12 }}>
                            Frontend: {p.frontend}
                          </span>
                        )}
                        {p.backend && (
                          <span style={{ background:'#fef9c3', color:'#854d0e', padding:'2px 10px', borderRadius:6, fontSize:12 }}>
                            Backend: {p.backend}
                          </span>
                        )}
                      </div>
                      ...
                      <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                        {p.students?.map(s => (
                          <span key={s._id} style={{ background:'#f0f4ff', padding:'3px 10px', borderRadius:6, fontSize:12 }}>
                            {s.name} ({s.enrollment || '-'})
                          </span>
                        ))}
                      </div>
          ...
          {tab === 'students' && (
            <div className="card" style={{ overflowX:'auto' }}>
              <table style={{ minWidth:800 }}>
                <thead>
                  <tr>
                    <th>#</th><th>Group</th><th>Name</th>
                    <th>Enrollment</th><th>Class</th><th>Email</th><th>Mobile</th>
                  </tr>
                </thead>
                <tbody>
                  {projects.flatMap((p, pi) =>
                    (p.students || []).map((s, si) => (
                      <tr key={p._id + s._id}>
                        <td>{pi * 10 + si + 1}</td>
                        <td>
                          <span style={{ background:'#eff6ff', color:'#1d4ed8', padding:'2px 8px', borderRadius:20, fontSize:12 }}>
                            {p.groupNo || '-'}
                          </span>
                        </td>
                  ...
                  {projects.length === 0 && (
                    <tr><td colSpan="7" style={{ textAlign:'center', color:'#888', padding:24 }}>No students</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          {/* Tab: Definitions — Full clear view */}
          {tab === 'definitions' && (
            <div>
              {projects.length === 0 && (
                <div className="card" style={{ textAlign:'center', color:'#888', padding:40 }}>
                ...
                <div className="card" key={p._id} style={{ marginBottom:20 }}>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16, paddingBottom:12, borderBottom:'1px solid #e5e7eb' }}>
                    <div>
                      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                        <h3 style={{ margin:0 }}>{p.title || 'Title not set'}</h3>
                        <span style={{ background:'#4f46e5', color:'white', padding:'2px 10px', borderRadius:20, fontSize:12, fontWeight:700 }}>
                          {p.groupNo || 'No Group'}
                        </span>
                      </div>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                      <span className={
                        ...
                        {p.definitionStatus === 'finalized' ? '✅ Finalized' :
                         p.definitionStatus === 'submitted' ? '📤 Submitted' : '⏳ Pending'}
                      </span>
                  ...
                  {(!p.definitions || p.definitions.length === 0) && (
                    <div style={{ textAlign:'center', color:'#aaa', padding:'20px 0', fontSize:14 }}>
                      Students have not submitted definitions yet.
                    </div>
                  )}

                  {/* All definitions listed clearly */}
                  {p.definitions?.map((d, i) => (
                    <div key={i} style={{
                      border: p.selectedDefinition === i ? '2px solid #4f46e5' : '1px solid #e5e7eb',
                      borderRadius:12, padding:20, marginBottom:12,
                      background: p.selectedDefinition === i ? '#f0f4ff' : 'white',
                      position:'relative'
                    }}>
                      {/* Selected badge */}
                      ...
                      <div style={{
                        ...
                        background: p.selectedDefinition === i ? '#4f46e5' : '#f3f4f6',
                        color: p.selectedDefinition === i ? 'white' : '#6b7280',
                      ...
                      <h4 style={{ margin:'0 0 8px', color:'#111', fontSize:16 }}>
                        {d.title}
                      </h4>
                      ...
                      <div style={{ display:'flex', gap:8 }}>
                        {d.frontend && (
                          <span style={{ background:'#eff6ff', color:'#1d4ed8', padding:'4px 12px', borderRadius:6, fontSize:13 }}>
                            🖥️ Frontend: {d.frontend}
                          </span>
                        )}
                        {d.backend && (
                          <span style={{ background:'#fef9c3', color:'#854d0e', padding:'4px 12px', borderRadius:6, fontSize:13 }}>
                            ⚙️ Backend: {d.backend}
                          </span>
                        )}
                      </div>
                  ...
                  {p.finalDefinition && (
                    <div style={{ background:'#f0fdf4', border:'2px solid #16a34a', borderRadius:10, padding:16, marginTop:8 }}>
                      ...
                      <p style={{ margin:'8px 0 0', color:'#333', fontSize:14, lineHeight:1.7 }}>
                        {p.finalDefinition}
                      </p>
                    </div>
                  )}
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999 }}>
          <div style={{ background:'white', borderRadius:16, padding:32, width:'100%', maxWidth:640, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <p style={{ color:'#888', fontSize:13, marginBottom:20 }}>
              Group: <strong>{selectModal.groupNo}</strong> |
              Choose one definition to finalize for this group.
            </p>
            ...
            <form onSubmit={finalizeDefinition}>
              <div style={{ marginBottom:20 }}>
                {selectModal.definitions?.map((d, i) => (
                  <div key={i}
                    ...
                    style={{
                      border: selectedIdx === i ? '2px solid #4f46e5' : '1px solid #e5e7eb',
                      borderRadius:10, padding:16, marginBottom:10, cursor:'pointer',
                      background: selectedIdx === i ? '#f0f4ff' : 'white',
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                      ...
                      <strong style={{ color: selectedIdx === i ? '#4f46e5' : '#111', fontSize:15 }}>
                        Definition {i+1}: {d.title}
                      </strong>
                    </div>
                    <p style={{ color:'#555', fontSize:13, lineHeight:1.7, margin:'0 0 8px 26px' }}>
                      {d.description}
                    </p>
                    <div style={{ display:'flex', gap:8, marginLeft:26 }}>
                      ...
                      {d.backend && (
                        <span style={{ background:'#fef9c3', color:'#854d0e', padding:'2px 8px', borderRadius:6, fontSize:12 }}>
                          BE: {d.backend}
                        </span>
                      )}
                    </div>
              ...
              <div className="form-group">
                ...
                <textarea rows="5"
                  ...
                  required
                  style={{ width:'100%', padding:'10px 14px', borderRadius:8, border:'1px solid #d1d5db', fontSize:14, lineHeight:1.6 }} />
              </div>
              ...
              <div style={{ display:'flex', gap:10, justifyContent:'flex-end', marginTop:16 }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={finalizing}>
                  {finalizing ? 'Finalizing...' : '✅ Finalize & Notify Students'}
                </button>
              </div>
            </form>
          </div>

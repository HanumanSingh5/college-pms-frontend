              ...
              <tbody>
                ...
                {faculties.map((f, i) => (
                  <tr key={f._id}>
                    ...
                    <td>
                      <span style={{ fontSize:13 }}>{f.email}</span>
                      <button type="button" onClick={() => copyText(f.email)}
                        title="Copy email"
                        style={{ marginLeft:6, background:'none', border:'none', cursor:'pointer', fontSize:12, color:'#6366f1' }}>
                        📋
                      </button>
                    </td>
                    <td>
                        ...
                        {f.plainPassword ? (
                          <>
                            <span style={{
                              ...
                              color:'#1e1b4b', background:'#e0e7ff',
                              padding:'3px 10px', borderRadius:6,
                            }}>
                              {visiblePasswords[f._id] ? f.plainPassword : '•'.repeat(f.plainPassword.length)}
                            </span>
                            <button type="button" onClick={() => togglePassword(f._id)}
                              title={visiblePasswords[f._id] ? 'Hide password' : 'Show password'}
                              style={{ background:'none', border:'none', cursor:'pointer', fontSize:14 }}>
                              {visiblePasswords[f._id] ? '🙈' : '👁️'}
                            </button>
                            <button type="button" onClick={() => copyText(f.plainPassword)}
                              title="Copy password"
                    ...
                    <td style={{ fontSize:13 }}>{new Date(f.createdAt).toLocaleDateString()}</td>
                    <td>
        ...
        <div
          ...
          <div style={{ background:'white', borderRadius:16, padding:32, width:'100%', maxWidth:480, boxShadow:'0 20px 60px rgba(0,0,0,0.3)', position:'relative', zIndex:10000 }}>
            ...
            <div style={{ background: editing ? '#fef9c3' : '#eff6ff', color: editing ? '#854d0e' : '#1d4ed8', padding:'10px 14px', borderRadius:8, marginBottom:16, fontSize:13, lineHeight:1.5 }}>
              {editing
                ? '✏️ Update faculty details. Leave password blank to keep existing password.'
                : '🔐 Credentials will be shown on screen after creation and saved in the table for future reference.'}
            </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {!editing && form.email && form.password && (
                <div style={{ background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:8, padding:'12px 14px', marginBottom:16, fontSize:13 }}>
                  <strong>📋 Credentials preview:</strong><br />
              ...
              <div style={{ display:'flex', gap:10, justifyContent:'flex-end', marginTop:20 }}>
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? 'Saving...' : editing ? 'Update Faculty' : 'Create Faculty Account'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div style={{ background:'white', borderRadius:16, padding:32, width:'100%', maxWidth:460, boxShadow:'0 20px 60px rgba(0,0,0,0.4)' }}>
            ...
            <div style={{ textAlign:'center', marginBottom:20 }}>
              ...
              <p style={{ color:'#666', fontSize:13, margin:'6px 0 0' }}>
                Share these credentials with the faculty manually. You can also view them anytime in the Faculties table.
              </p>
            </div>
            ...
            <div style={{ background:'#f0f4ff', border:'2px solid #c7d2fe', borderRadius:12, overflow:'hidden', marginBottom:20 }}>
              <div style={{ padding:'14px 20px', borderBottom:'1px solid #e0e7ff' }}>
                ...
                <p style={{ margin:'4px 0 0', fontSize:14, color:'#4f46e5', fontWeight:600 }}>{window.location.origin}/login</p>
              </div>
              <div style={{ padding:'14px 20px', borderBottom:'1px solid #e0e7ff' }}>
                <p style={{ margin:0, fontSize:11, color:'#6366f1', fontWeight:700, textTransform:'uppercase', letterSpacing:1 }}>Email</p>
                <p style={{ margin:'4px 0 0', fontSize:16, fontWeight:700, color:'#1e1b4b', fontFamily:'monospace' }}>{newCredentials.email}</p>
              </div>

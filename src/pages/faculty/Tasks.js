...
export default function FacultyTasks() {
  ...
  const load = () => {
    axios.get('http://localhost:5000/api/faculty/tasks',           h).then(r => setTasks(r.data));
    axios.get('http://localhost:5000/api/faculty/late-submissions', h).then(r => setLateTasks(r.data));
  ...
  const createTask = async (e) => {
    ...
    try {
      await axios.post('http://localhost:5000/api/faculty/task', form, h);
  ...
  const saveEdit = async (e) => {
    e.preventDefault();
    try {
      await axios.put('http://localhost:5000/api/faculty/task/' + editModal._id, editForm, h);
  ...
  const deleteTask = async (id) => {
    ...
    await axios.delete('http://localhost:5000/api/faculty/task/' + id, h);
  ...
  const toggleUpload = async (task) => {
    try {
      const res = await axios.put(
        'http://localhost:5000/api/faculty/task/' + task._id + '/enable-upload',
                ...
                <div key={project?._id} style={{ marginBottom:24 }}>
                  ...
                  {ptasks.map(t => (
                    <div key={t._id} className="card" style={{
                      marginBottom:10,
                      borderLeft: t.status === 'late' ? '4px solid #ef4444' :
                                  t.status === 'completed' ? '4px solid #16a34a' :
                                  isOverdue(t.dueDate) ? '4px solid #f97316' : '4px solid #e5e7eb'
                    }}>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                        <div style={{ flex:1 }}>
                          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6, flexWrap:'wrap' }}>
                            {t.phase && (
                              <span style={{ background:'#f0f4ff', color:'#4f46e5', padding:'2px 10px', borderRadius:20, fontSize:12, fontWeight:600 }}>
                                {PHASES.find(p => p.value === t.phase)?.label || t.phase}
                              </span>
                            )}
                            ...
                            {isOverdue(t.dueDate) && t.status !== 'completed' && (
                              <span style={{ background:'#fee2e2', color:'#dc2626', padding:'2px 8px', borderRadius:20, fontSize:11, fontWeight:600 }}>
                                OVERDUE
                              </span>
                            )}
                            <span style={{
                              ...
                              {t.uploadEnabled ? '🔓 Upload ON' : '🔒 Upload OFF'}
                            </span>
                          </div>
                          ...
                          <div style={{ display:'flex', gap:16, fontSize:12, color:'#888', flexWrap:'wrap' }}>
                            <span style={{
                              color: isOverdue(t.dueDate) && t.status !== 'completed' ? '#dc2626' : '#888',
                              fontWeight: isOverdue(t.dueDate) && t.status !== 'completed' ? 600 : 400
                            }}>
                              📅 Due: {t.dueDate ? new Date(t.dueDate).toLocaleDateString() : 'No date'}
                              {isOverdue(t.dueDate) && t.status !== 'completed' && ' ⚠️'}
                            </span>
                        ...
                        <div style={{ display:'flex', flexDirection:'column', gap:6, marginLeft:16 }}>
                          ...
                          <button type="button"
                            onClick={() => toggleUpload(t)}
                            style={{
                              padding:'5px 10px', fontSize:12, borderRadius:8, cursor:'pointer', fontWeight:600, border:'none',
                    ...
                    <table style={{ width:'100%', minWidth:800 }}>
                      <thead>
                        <tr style={{ background:'#fef2f2' }}>
                          ...
                          <th style={{ padding:'10px 12px', textAlign:'left', fontSize:13, color:'#dc2626' }}>Group No.</th>
                          <th style={{ padding:'10px 12px', textAlign:'left', fontSize:13, color:'#dc2626' }}>Task</th>
                          <th style={{ padding:'10px 12px', textAlign:'left', fontSize:13, color:'#dc2626' }}>Students</th>
                      ...
                      <tbody>
                        {lateTasks.map((t, i) => {
                          const daysLate = t.dueDate
                            ? Math.ceil((new Date() - new Date(t.dueDate)) / (1000*60*60*24))
                            : 0;
                          return (
                            <tr key={t._id} style={{ borderBottom:'1px solid #fee2e2', background:'#fff5f5' }}>
                              ...
                              <td style={{ padding:'10px 12px' }}>
                                ...
                                <small style={{ color:'#888' }}>{t.phase || 'No phase'}</small>
                              </td>
                              ...
                              <td style={{ padding:'10px 12px' }}>
                                <span style={{ background:'#fee2e2', color:'#dc2626', padding:'3px 10px', borderRadius:20, fontSize:12, fontWeight:700 }}>
                              ...
                              <td style={{ padding:'10px 12px' }}>
                                <button type="button"
                                  onClick={() => toggleUpload(t)}
                                  style={{
                                    padding:'5px 12px', fontSize:12, borderRadius:6, cursor:'pointer', fontWeight:600, border:'none',
                                    background: t.uploadEnabled ? '#fef3c7' : '#f0fdf4',
                                    color: t.uploadEnabled ? '#92400e' : '#16a34a',
                                  }}>
          ...
          <div style={{ background:'white', borderRadius:16, padding:32, width:'100%', maxWidth:520, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <form onSubmit={createTask}>
                ...
                <select value={form.projectId} onChange={e => setForm({...form, projectId: e.target.value})} required>
                  ...
                  {projects.filter(p => p.definitionStatus === 'finalized').map(p => (
                    <option key={p._id} value={p._id}>
                      [{p.groupNo || '-'}] {p.title || 'Untitled'} ({p.students?.length || 0} students)
                    </option>
                  ))}
                </select>
                {projects.filter(p => p.definitionStatus === 'finalized').length === 0 && (
                  <p style={{ color:'#dc2626', fontSize:12, marginTop:4 }}>No finalized projects. Please finalize a definition first.</p>
                )}
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input type="text" placeholder="e.g. Submit Abstract Document"
                  value={form.title} onChange={e => setForm({...form, title: e.target.value})} required />
              </div>
              <div className="form-group">
                <label>Task Description *</label>
                <textarea rows="3" placeholder="What students need to submit..."
                ...
                <div style={{ background:'#f0f4ff', padding:'10px 14px', borderRadius:8, marginBottom:16, fontSize:13 }}>
                  ...
                  <div style={{ marginTop:4 }}>
                    {projects.find(p => p._id === form.projectId)?.students?.map(s => (
                      <span key={s._id} style={{ display:'inline-block', background:'white', padding:'2px 8px', borderRadius:20, fontSize:12, margin:'2px 4px 2px 0', border:'1px solid #c7d2fe' }}>
                        {s.name}
                      </span>
                    ))}
                  </div>
            ...
            {viewTask.submissions?.length === 0 ? (
              ...
              viewTask.submissions.map((sub, i) => (
                <div key={i} style={{ border:'1px solid #e5e7eb', borderRadius:10, padding:16, marginBottom:10 }}>
                  ...
                  <a href={'http://localhost:5000/uploads/' + sub.document}

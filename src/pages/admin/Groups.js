...
export default function AdminGroups() {
  ...
  const load = () => {
    axios.get('http://localhost:5000/api/admin/student-groups', h)
      .then(r => setGroups(r.data)).catch(() => {});
    axios.get('http://localhost:5000/api/admin/faculties', h)
      .then(r => setFaculties(r.data)).catch(() => {});
  };
  ...
  const assignGroup = async (e) => {
    ...
    setSaving(true);
    try {
      await axios.post('http://localhost:5000/api/admin/assign-group', {
  ...
  const saveTeam = async (e) => {
    ...
    try {
      await axios.put(
        'http://localhost:5000/api/admin/student/' + editTeamModal.student._id + '/team',
  ...
  const removeMember = (i) => {
    setTeamForm(teamForm.filter((_, idx) => idx !== i));
  };
  ...
  const displayList =
    tab === 'groups'      ? filtered :
    tab === 'unassigned'  ? unassigned : assigned;
      ...
      <div className="layout">
        ...
        <div className="main-content">
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
            <div>
              ...
              <p style={{ color:'#888', fontSize:14, margin:'4px 0 0' }}>
                Each student with a team forms a group. Assign groups to faculty.
              </p>
            </div>
          ...
          <div style={{ borderBottom:'1px solid #e5e7eb', marginBottom:16, display:'flex', gap:4 }}>
            <button style={tabStyle('groups')} onClick={() => setTab('groups')} type="button">
              All Groups ({groups.length})
            </button>
            <button style={tabStyle('unassigned')} onClick={() => setTab('unassigned')} type="button">
              Unassigned ({unassigned.length})
            </button>
            <button style={tabStyle('assigned')} onClick={() => setTab('assigned')} type="button">
          ...
          <input
            type="text"
            placeholder="Search by student name, enrollment or group number..."
            value={search}
            ...
            <div className="card" key={g.student._id} style={{ marginBottom:16 }}>
                ...
                <div>
                  <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:4 }}>
                    ...
                    <span style={{ fontWeight:600, fontSize:16 }}>
                      {g.student.name}'s Group
                    </span>
                    <span style={{
                      background: g.faculty ? '#dcfce7' : '#fef9c3',
                      color: g.faculty ? '#166534' : '#854d0e',
                    ...
                    <span style={{
                      background:
                        g.definitionStatus === 'finalized' ? '#dcfce7' :
                        g.definitionStatus === 'submitted' ? '#dbeafe' : '#f3f4f6',
                      color:
                        g.definitionStatus === 'finalized' ? '#166534' :
                        g.definitionStatus === 'submitted' ? '#1e40af' : '#6b7280',
                      padding:'2px 10px', borderRadius:20, fontSize:12, fontWeight:600
                    }}>
                      {g.definitionStatus === 'finalized' ? '✅ Def Finalized' :
                       g.definitionStatus === 'submitted' ? '📝 Defs Submitted' : '⏳ No Definitions'}
                    </span>
                  </div>
              ...
              {g.definitions && g.definitions.length > 0 && (
                <div style={{
                  ...
                  marginTop:10, fontSize:13, color:'#4f46e5',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ color:'#666', fontSize:14, marginBottom:20 }}>
              Student: <strong>{assignModal.student.name}</strong> |
              Team: <strong>{assignModal.teamMembers?.length || 0} members</strong>
            </p>
            ...
            <form onSubmit={assignGroup}>
              <div className="form-group">
                ...
                <input
                  type="text"
                  placeholder="e.g. G-01, Group 1, Batch-A"
                  value={assignForm.groupNo}
                  onChange={e => setAssignForm({...assignForm, groupNo: e.target.value})}
                  required
                />
              </div>
              <div className="form-group">
              ...
              <div style={{
                ...
                marginBottom:16, fontSize:13, color:'#1e40af'
              }}>
                After assigning, faculty will be able to see this group's definitions
                and select one to finalize.
              </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
              <div>
                ...
                <p style={{ color:'#888', fontSize:13, margin:'4px 0 0' }}>
                  {viewDefs.definitions?.length} definition(s) submitted
                </p>
              </div>
              <button type="button" className="btn"
                onClick={() => setViewDefs(null)}
                style={{ background:'#e5e7eb' }}>
                Close
              </button>
            </div>
            ...
            {viewDefs.definitions?.map((d, i) => (
              <div key={i} style={{
                border: viewDefs.project?.selectedDefinition === i
                  ? '2px solid #4f46e5' : '1px solid #e5e7eb',
                borderRadius:12, padding:16, marginBottom:12,
                background: viewDefs.project?.selectedDefinition === i ? '#f0f4ff' : 'white'
              }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                  <strong style={{ color: viewDefs.project?.selectedDefinition === i ? '#4f46e5' : '#111' }}>
                    Definition {i+1}: {d.title}
                  </strong>
                  {viewDefs.project?.selectedDefinition === i && (
                    <span style={{
                      background:'#dcfce7', color:'#166534',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ marginBottom:4 }}>
              Edit Team — {editTeamModal.student.name}
            </h3>
            <p style={{ color:'#888', fontSize:13, marginBottom:20 }}>
              Add, edit or remove team members for this group.
            </p>
              ...
              {teamForm.map((m, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                    <strong style={{ color:'#4f46e5', fontSize:14 }}>
                      Member {i + 1}
                      {i === 0 && (
                        <span style={{
                          background:'#ede9fe', color:'#5b21b6',
                          fontSize:11, padding:'2px 8px', borderRadius:20, marginLeft:8, fontWeight:400
                        }}>
                          Primary
                        </span>
                      )}
                    ...
                    {[
                      ...
                      <div key={f}>
                        <label style={{
                          ...
                          marginBottom:4, fontWeight:500, color:'#374151'
                        }}>
                          {l}
                        </label>
                        <input
                          ...
                          placeholder={p}
                          value={m[f] || ''}
                          onChange={e => updateMember(i, f, e.target.value)}
                          style={{

...
export default function AdminProjects() {
  ...
  const load = () => {
    ...
    axios.get('http://localhost:5000/api/admin/faculties', h).then(r => setFaculties(r.data)).catch(()=>{});
    axios.get('http://localhost:5000/api/admin/students',  h).then(r => setStudents(r.data)).catch(()=>{});
  };
  ...
  const save = async (e) => {
    ...
    try {
      if (editProject) {
        await axios.put('http://localhost:5000/api/admin/project/' + editProject._id, form, h);
        toast.success('Project updated!');
      } else {
        await axios.post('http://localhost:5000/api/admin/project', form, h);
  ...
  const remove = async (id) => {
    ...
    try {
      await axios.delete('http://localhost:5000/api/admin/project/' + id, h);
  ...
  // Get all unique categories in projects
              ...
              return (
                <button key={cat} type="button"
                  ...
                  style={{
                    ...
                    border: activeCategory === cat ? '2px solid #4f46e5' : '1px solid #e5e7eb',
                    background: activeCategory === cat ? '#4f46e5' : colorInfo.bg,
                    color: activeCategory === cat ? 'white' : colorInfo.color,
                  }}>
                  {cat} ({count})
                </button>
              );
              ...
              {CATEGORIES.filter(cat => projects.some(p => (p.category||'Other') === cat)).map(cat => {
                ...
                return (
                  <div key={cat}
                    onClick={() => setActiveCategory(cat)}
                    style={{
                      background: colorInfo.bg, borderRadius:10, padding:'14px 16px',
                      cursor:'pointer', border: '1px solid ' + colorInfo.color + '33',
            ...
            <table style={{ minWidth:1300, borderCollapse:'collapse', width:'100%' }}>
              <thead>
                <tr>
                  ...
                  <th style={thStyle}>Frontend</th>
                  <th style={thStyle}>Backend</th>
                  <th style={thStyle}>Faculty Guide</th>
              ...
              <tbody>
                {rows.length === 0 && (
                  <tr>
                    <td colSpan="13" style={{ textAlign:'center', color:'#888', padding:32 }}>
                ...
                {rows.map((row, i) => {
                  ...
                  return (
                    <tr key={row.projectId + '-' + i}
                      style={{ background: row.isMain === false ? '#fafafa' : 'white', borderBottom: row.isFirst && i > 0 ? '2px solid #d1d5db' : '1px solid #f0f0f0' }}>
                      <td style={tdStyle}>{row.srNo && <span style={{ fontWeight:700, color:'#4f46e5' }}>{row.srNo}</span>}</td>
                      <td style={tdStyle}>
                        {row.isFirst && (
                          <span style={{ background:'#4f46e5', color:'white', padding:'3px 10px', borderRadius:20, fontSize:11, fontWeight:700 }}>
                            {row.groupNo}
                          </span>
                        )}
                      </td>
                      <td style={tdStyle}>
                        {row.isFirst && row.category !== '-' && (
                          <span style={{ background: catColor.bg, color: catColor.color, padding:'3px 8px', borderRadius:6, fontSize:11, fontWeight:600 }}>
                            {row.category}
                          </span>
                        )}
                      </td>
                      <td style={tdStyle}>
                        <span style={{ background: row.isMain===false?'#f3f4f6':'#eff6ff', color: row.isMain===false?'#6b7280':'#1d4ed8', padding:'2px 6px', borderRadius:4, fontSize:11, fontWeight:600 }}>
                          {row.enrollment}
                        </span>
                      </td>
                      <td style={tdStyle}>
                        <strong>{row.name}</strong>
                        {row.isMain === false && <span style={{ background:'#f3f4f6', color:'#6b7280', fontSize:10, padding:'1px 5px', borderRadius:8, marginLeft:4 }}>member</span>}
                      </td>
                      ...
                      <td style={tdStyle}>
                        {row.isFirst && row.frontend !== '-' && (
                          <span style={{ background:'#eff6ff', color:'#1d4ed8', padding:'2px 8px', borderRadius:6, fontSize:11, fontWeight:600 }}>{row.frontend}</span>
                        )}
                        {row.isFirst && row.frontend === '-' && <span style={{ color:'#aaa', fontSize:11 }}>-</span>}
                      </td>
                      <td style={tdStyle}>
                        {row.isFirst && row.backend !== '-' && (
                          <span style={{ background:'#fef9c3', color:'#854d0e', padding:'2px 8px', borderRadius:6, fontSize:11, fontWeight:600 }}>{row.backend}</span>
                        )}
                        {row.isFirst && row.backend === '-' && <span style={{ color:'#aaa', fontSize:11 }}>-</span>}
                      </td>
                      <td style={tdStyle}>{row.isFirst && <span style={{ fontSize:12 }}>{row.faculty}</span>}</td>
                      <td style={tdStyle}>
                        {row.isFirst && (
                          <div style={{ display:'flex', gap:4 }}>
                            <button type="button" className="btn btn-warning" onClick={() => openEdit(row.project)} style={{ padding:'4px 8px', fontSize:11 }}>Edit</button>
                            <button type="button" className="btn btn-danger" onClick={() => remove(row.projectId)} style={{ padding:'4px 8px', fontSize:11 }}>Delete</button>
                          </div>
                        )}
                      </td>
                    </tr>
            ...
            <form onSubmit={save}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                <div className="form-group">
                  ...
                  <input type="text" placeholder="e.g. G-01" value={form.groupNo}
                    onChange={e => setForm({...form, groupNo: e.target.value})} required />
                </div>
                <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input type="text" placeholder="Enter project title" value={form.title}
              ...
              <div className="form-group">
                ...
                <textarea rows="3" placeholder="Detailed definition" value={form.definition}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                <div className="form-group">
                  ...
                  <input type="text" placeholder="e.g. React" value={form.frontend}
                    onChange={e => setForm({...form, frontend: e.target.value})} />
                </div>
                <div className="form-group">
                  <label>Backend</label>
                  <input type="text" placeholder="e.g. Node.js" value={form.backend}
                  ...
                  {students.map(s => (
                    <label key={s._id} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6, cursor:'pointer', fontSize:13 }}>
                      ...
                      {s.teamMembers?.length > 0 && (
                        <span style={{ background:'#eff6ff', color:'#1d4ed8', fontSize:10, padding:'1px 6px', borderRadius:10 }}>
                          +{s.teamMembers.length} members
                        </span>
                      )}
                    </label>
                  ))}
